...
import { IndianRupee, Search, TrendingUp, Compass, ArrowUpRight, ArrowDownRight, RefreshCw, BarChart2, ExternalLink } from 'lucide-react';
...
const MarketPrices: React.FC = () => {
  ...
  const getPriceDifference = (today: number, yesterday: number) => {
    ...
    return <span className="text-xs font-bold text-slate-500">₹0 ({t('mandi.stable', 'Stable')})</span>;
  };
  ...
  const getChartData = () => {
    ...
    return {
      labels: [
        t('mandi.daysAgo6', '6 Days Ago'),
        t('mandi.daysAgo5', '5 Days Ago'),
        t('mandi.daysAgo4', '4 Days Ago'),
        t('mandi.daysAgo3', '3 Days Ago'),
        t('mandi.daysAgo2', '2 Days Ago'),
        t('mandi.yesterday', 'Yesterday'),
        t('mandi.today', 'Today')
      ],
      datasets: [
        {
          label: `${selectedCrop.cropName} ${t('mandi.priceTrend', 'Price Trend')} (₹/${t('mandi.quintal', 'Quintal')})`,
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        >
          ...
          {t('mandi.refreshPrices', 'Refresh Prices')}
        </button>
      </div>
      ...
      {bestMarket && (
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="glass-panel p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-2">
            <span className="text-xs text-slate-400 dark:text-slate-500 uppercase font-bold tracking-wider">{t('mandi.bestPrice')}</span>
            <h3 className="text-2xl font-extrabold text-slate-900 dark:text-white">
              {bestMarket.cropName} (₹{bestMarket.todayPrice})
            </h3>
            <p className="text-xs text-slate-500">{t('common.mandiName', 'Market (Mandi)')}: {bestMarket.marketName}, {bestMarket.district}</p>
          </div>

          <div className="glass-panel p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-2">
            <span className="text-xs text-slate-400 dark:text-slate-500 uppercase font-bold tracking-wider">{t('mandi.topTrading', 'Top Trading Crop')}</span>
            <h3 className="text-2xl font-extrabold text-primary-600 dark:text-primary-400">Wheat</h3>
            <p className="text-xs text-slate-500">{t('mandi.topTradingDesc', 'Highest volume reported across regional collections')}</p>
          </div>
          ...
          <div className="glass-panel p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-2 col-span-1 sm:col-span-2 lg:col-span-1">
            <span className="text-xs text-slate-400 dark:text-slate-500 uppercase font-bold tracking-wider">{t('mandi.trendOutlook', 'Trend Outlook')}</span>
            <h3 className="text-2xl font-extrabold text-emerald-600 dark:text-emerald-400">{t('mandi.trendUpward', 'Upward (+1.8%)')}</h3>
            <p className="text-xs text-slate-500">{t('mandi.trendOutlookDesc', 'Prices showing positive growth patterns this week')}</p>
          </div>
                ...
                <table className="w-full text-left text-sm border-collapse">
                  <thead>
                    <tr className="bg-slate-100/50 dark:bg-slate-900/40 text-slate-500 text-xs font-bold border-b border-slate-150 dark:border-slate-800">
                      <th className="p-4">{t('mandi.crop')}</th>
                      <th className="p-4">{t('mandi.mandi')}</th>
                      <th className="p-4">{t('mandi.todayPrice')}</th>
                      <th className="p-4">{t('mandi.trendShift', 'Trend Shift')}</th>
                    </tr>
                  </thead>
                      ...
                      prices.map((crop) => (
                        ...
                        >
                          ...
                          <td className="p-4 font-extrabold text-slate-850 dark:text-slate-100">
                            ₹{crop.todayPrice} <span className="text-slate-400 text-xs font-normal">/ {t('mandi.quintal', 'Quintal')}</span>
                          </td>
                          <td className="p-4">
        ...
        <div className="lg:col-span-5 space-y-4">
          {selectedCrop ? (
            <div className="glass-panel p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-md space-y-6">
              ...
              <div className="border-b border-slate-100 dark:border-slate-850 pb-3">
                <h3 className="text-lg font-bold flex items-center gap-1.5 text-slate-900 dark:text-white">
                  ...
                  {t('mandi.weeklyTrends', 'Weekly Trends')}
                </h3>
                <p className="text-xs text-slate-500">
                  {t('mandi.weeklyTrendsDesc', 'Historical performance metrics')} - {selectedCrop.cropName} ({selectedCrop.marketName})
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 pt-2 text-xs">
                <div className="p-3 bg-slate-50 dark:bg-slate-900/40 rounded-xl border border-slate-100 dark:border-slate-850/50">
                  <span className="text-slate-400 block mb-0.5">{t('mandi.todayPrice')}</span>
                  <h4 className="text-lg font-extrabold">₹{selectedCrop.todayPrice}</h4>
                </div>
                <div className="p-3 bg-slate-50 dark:bg-slate-900/40 rounded-xl border border-slate-100 dark:border-slate-850/50">
                  <span className="text-slate-400 block mb-0.5">{t('mandi.yesterdayPrice')}</span>
            ...
            <div className="glass-panel rounded-3xl p-12 text-center border border-slate-200 dark:border-slate-800 flex flex-col items-center justify-center space-y-4">
              ...
              <h3 className="text-lg font-bold">{t('mandi.selectCropListing', 'Select a crop listing.')}</h3>
              <p className="text-sm text-slate-500 max-w-sm">
                {t('mandi.selectCropListingDesc', 'Tap on a mandi rate listing on the left to examine detailed charts and historical trends.')}
              </p>
            </div>
          )}
        </div>
